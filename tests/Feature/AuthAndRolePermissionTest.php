...
test('packer role cannot access warehouse products or mutations and receives 403', function () {
    $packer = User::where('email', 'packer@vendora.id')->first() ?? User::where('email', 'packer@medina.com')->first();
...
test('super admin can access all menus including role and user management', function () {
    $admin = User::where('email', 'admin@vendora.id')->first() ?? User::where('email', 'admin@medina.com')->first();
