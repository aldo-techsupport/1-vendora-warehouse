...
{
    /**
     * Get effective AI configuration (priority: Backend Cloud synchronized config, fallback: local config).
     */
    public function getEffectiveConfig(): array
    {
        try {
            $licenseService = app(LicenseService::class);
            $dynamicConfig = $licenseService->getAiConfig();
            if (!empty($dynamicConfig) && !empty($dynamicConfig['base_url'])) {
                return [
                    'base_url' => $dynamicConfig['base_url'],
                    'api_key' => $dynamicConfig['api_key'] ?? config('ai.api_key', ''),
                    'model' => $dynamicConfig['model'] ?? config('ai.model', 'qmodel_38max'),
                    'timeout' => (int) ($dynamicConfig['timeout'] ?? config('ai.timeout', 60)),
                    'temperature' => (float) ($dynamicConfig['temperature'] ?? config('ai.temperature', 0.7)),
                    'is_dynamic' => true,
                ];
            }
        } catch (\Throwable $e) {
        ...
        return [
            'base_url' => config('ai.base_url', 'https://vendorarouter.web.id/api/v1'),
            'api_key' => config('ai.api_key', ''),
            'model' => config('ai.model', 'qmodel_38max'),
    ...
    public function generateSalesAnalysis(?int $userId = null): AiAnalysis
    {
        ...
        try {
            ...
            $response = $this->sendChatCompletion([
                ['role' => 'system', 'content' => 'Anda adalah konsultan senior e-commerce dan pakar strategi pemasaran ritel marketplace Shopee untuk "Vendora Shopee Management". Berikan analisis mendalam, tajam, profesional, dan dapat langsung dieksekusi. Berikan output HANYA dalam format JSON valid tanpa teks lain.'],
    ...
    public function testConnection(): array
    {
        ...
        if (empty($cfg['api_key'])) {
            return [
                'success' => false,
                'message' => 'API Key belum dikonfigurasi pada sistem pusat.',
    ...
    protected function sendChatCompletion(array $messages, bool $jsonMode = false): array
    {
        ...
        $headers = [
            ...
            'X-Title' => config('ai.site_name', 'Vendora Shopee Management'),
        ];
    ...
    protected function buildAnalysisPrompt(array $metrics): string
    {
        ...
        return <<<PROMPT
Analisis data penjualan dan inventaris toko "Vendora Warehouse" berikut ini:
...
1. Anda adalah "Asisten Analisis Penjualan & Pemasaran Vendora Warehouse".
2. DILARANG KERAS mengaku atau menyebut diri sebagai asisten koding, pemrograman, developer perangkat lunak, Qoder, atau menyebut nama model teknis AI. Anda adalah asisten dan penasihat bisnis operasional toko Vendora Warehouse.
...
Anda adalah Asisten Analisis Penjualan & Pemasaran Vendora Warehouse (BUKAN asisten pemrograman/koding, JANGAN sebut diri Anda Qoder atau asisten koding).
Anda MEMILIKI data toko Vendora Warehouse hari ini:
...
Tolong jawab pertanyaan seller berikut ini secara langsung sebagai Asisten Penjualan Vendora dengan memberikan langkah konkret dan solusi:
CTX;
    }
    ...
    protected function generateFallbackAnalysis(array $metrics, ?int $userId, string $modelName): AiAnalysis
    {
        ...
        $summary = "Berdasarkan evaluasi sistem pada data toko Vendora Warehouse, tercatat total {$totalOrders} pesanan Shopee dengan akumulasi omzet {$totalRevenueRp}. "
        ...
        $marketingAdvice = [
            ...
            [
                ...
                'description' => 'Kirimkan pesan broadcast chat kepada pelanggan yang pernah berbelanja di Vendora Warehouse dengan menawarkan voucher eksklusif potongan belanja.',
    ...
    protected function generateSmartChatFallback(string $message, array $metrics): string
    {
        ...
        if (str_contains($msgLower, 'bundling') || str_contains($msgLower, 'paket') || str_contains($msgLower, 'promo')) {
            return "🎁 **Rekomendasi Paket Bundling & Promo Shopee:**\n\n"
                ."1. **Paket OOTD Eksklusif Vendora:**\n"
                ."   - *Kombinasi:* Gamis Silk Premium (`VND-GMS-001`) + Hijab Pashmina Plisket (`VND-HJB-001`)\n"
                ...
                ."   - *Kombinasi:* Kemeja Koko Kurta (`VND-KOKO-001`) + Sarung Tenun Jacquard (`VND-SRG-001`)\n"
        ...
        if (str_contains($msgLower, 'broadcast') || str_contains($msgLower, 'copy') || str_contains($msgLower, 'pesan')) {
            return "📢 **Draft Pesan Broadcast Chat Shopee:**\n\n"
                ."*Subjek:* Promo Spesial Khusus Pelanggan Setia Vendora Warehouse! ✨\n\n"
                ."*Halo Kak!* Terima kasih sudah menjadi bagian dari keluarga Vendora. Ada kabar gembira khusus hari ini! 🎉\n\n"
                ."Dapatkan **Voucher Diskon Tambahan 10%** tanpa minimum belanja untuk koleksi produk pilihan kami. Gunakan kode voucher: **VENDORASPECIAL** saat checkout.\n\n"
        ...
        return "Halo! Saya adalah **Vendora AI Seller Advisor**. Saya dapat membantu menganalisis data penjualan toko, memberikan ide promosi Shopee, saran bundling produk, serta peringatan stok yang perlu di-restock.\n\n"
